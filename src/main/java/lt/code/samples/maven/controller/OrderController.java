...
public class OrderController {
    ...
    // Orderio sukūrimas
    @PostMapping("/new")
    public String submitOrder(@ModelAttribute OrderFormDTO orderForm, RedirectAttributes redirectAttributes) {
    ...
    //!!! LABAI SVARBU: SPECIFIŠKESNI ENDPOINTAI VISADA VIRŠUJE

    // ----------- WORKLOG ENDPOINTAI -----------

    @GetMapping(value = "/worklog")
    public String showWorkLog(
        ...
        if (orderId != null) {
            return workLogService.findOrderById(orderId)
                    ...
                    .orElseGet(() -> {
                        ra.addFlashAttribute("message", "Order with ID " + orderId + " not found.");
        ...
        // Jei be param, tiesiog tuščia paieškos forma
        return "orders/worklog";
    }
    ...
    public String endWork(@RequestParam Long workLogId, RedirectAttributes redirectAttributes) {
        ...
        if (log.isFaulty() && !log.isFaultFixed()) {
            redirectAttributes.addFlashAttribute("error", "Cannot finish a faulty operation.");
    ...
    public String endOrder(@RequestParam Long orderId,
        ...
        if (!allFinished) {
            ra.addFlashAttribute("error", "Cannot end order – some tasks are unfinished or faulty.");
            return "redirect:/orders/worklog?orderName=" + order.getOrderName();
        }
